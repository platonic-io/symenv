...
install_symenv_from_git() {
  ...
  command git -c advice.detachedHead=false --git-dir="$INSTALL_DIR"/.git --work-tree="$INSTALL_DIR" checkout -f --quiet FETCH_HEAD 2>/dev/null \
    || {
...
symenv_do_install() {
  ...
  SYMENV_PROFILE="$(symenv_detect_profile)"
